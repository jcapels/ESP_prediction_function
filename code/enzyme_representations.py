...
def calculate_esm1b_ts_vectors(enzyme_df):

	#creating model input:
	df_enzyme = preprocess_enzymes_df(enzyme_df)
	#loading ESM-1b model:
	print(".....2(a) Loading ESM-1b model.")
	#model, alphabet = esm.pretrained.esm1b_t33_650M_UR50S()
	model_location = join("..", "data", "saved_models", "ESM1b", "esm1b_t33_650M_UR50S.pt")

	model_data = torch.load(model_location, map_location='cuda:1')
	regression_location = model_location[:-3] + "-contact-regression.pt"

	regression_data = torch.load(regression_location, map_location='cuda:1')
	...
	model_dict = torch.load(PATH, map_location=torch.device('cuda:1'))
	...
	for i, row in tqdm(df_enzyme.iterrows(), total=df_enzyme.shape[0]):
		model_input = row["model_input"]
		ind = row["Enzyme ID"]
		if validate_enzyme(seq = model_input):
			batch_labels, batch_strs, batch_tokens = batch_converter([(ind, model_input)])
			with torch.no_grad():
				results = model(batch_tokens, repr_layers=[33])
			results_enzymes[ind] = results["representations"][33][0][0].numpy()
	return results_enzymes
...
def calcualte_esm1b_ts_vectors(enzyme_list):
	...
	for ind in tqdm(df_enzyme.index):
	...
	return(df_enzyme), results_enzymes
...
def preprocess_enzymes_df(df_enzyme):
	#enzyme_list = [enz for enz in enzyme_list if type(enz) == str]
	#if length of sequence is longer than 1020 amino acids, we crop it:
	df_enzyme["model_input"] = [seq[:1022].replace("\r", "").replace("\r", "").replace("\n", "") for seq in df_enzyme["Sequence"]]
	return(df_enzyme)
